    ...
    for params in param_dicts:
        ...
        if cv:
            for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train)):
                ...
                if use_range_conversion:
                    # Transform true values to class indices for metrics
                    y_val_true_class = model._convert_to_classes(y_val, model.range_min, model.range_max)
                    y_tr_true_class = model._convert_to_classes(y_tr, model.range_min, model.range_max)
                    y_val_pred = y_val_pred_idx
                    y_val_orig = y_val_true_class
                    y_tr_pred = y_tr_pred_idx
                    y_tr_orig = y_tr_true_class
                else:
                ...
                for target_col, (min_val, max_val) in target_ranges.items():
                    th_metrics_valid = compute_threshold_metrics(y_val_orig, y_val_pred, min_val, max_val, class_mapping=model.class_mapping if use_range_conversion else None)
                    th_metrics_train = compute_threshold_metrics(y_tr_orig, y_tr_pred, min_val, max_val, class_mapping=model.class_mapping if use_range_conversion else None)
            ...
            if use_range_conversion:
                ...
                # Transform true values to class indices for metrics
                y_test_true_class = model._convert_to_classes(y_test_encoded, model.range_min, model.range_max)
            else:
            ...
            for target_col, (min_val, max_val) in target_ranges.items():
                th_metrics_test = compute_threshold_metrics(y_test_orig, y_test_pred, min_val, max_val, class_mapping=model.class_mapping if use_range_conversion else None)
            ...
            if use_range_conversion:
                ...
                # Transform true values to class indices for metrics
                y_test_true_class = model._convert_to_classes(y_test_encoded, model.range_min, model.range_max)
                y_train_true_class = model._convert_to_classes(y_train_encoded, model.range_min, model.range_max)
            else:
                model.fit(X_train, y_train_encoded)
                y_test_pred_proba = model.predict_proba(X_test)
            ...
            if use_range_conversion:
                ...
                y_train_orig = y_train_true_class
            else:
                if idx_to_class is not None:
                    y_test_pred = np.vectorize(idx_to_class.get)(y_test_pred_idx)
                    y_test_orig = np.vectorize(idx_to_class.get)(y_test_encoded)
            ...
            for target_col, (min_val, max_val) in target_ranges.items():
                th_metrics_test = compute_threshold_metrics(y_test_orig, y_test_pred, min_val, max_val, class_mapping=model.class_mapping if use_range_conversion else None)
                th_metrics_train = compute_threshold_metrics(y_train_orig, y_train_pred, min_val, max_val, class_mapping=model.class_mapping if use_range_conversion else None)
                for k_, v_ in th_metrics_test.items():
                    test_threshold_metrics.setdefault(f"{target_col}_test_{k_}", []).append(v_)
                for k_, v_ in th_metrics_train.items():
                    train_threshold_metrics.setdefault(f"{target_col}_train_{k_}", []).append(v_)
        ...
        if return_final_model:
            final_models[str(params)] = model
    if return_final_model:
        return metrics_dict, final_models
    else:
